  ...
  return (
    <DirectionProvider direction={direction}>
              ...
              {ABAYA_SIZES.map((size) => {
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      update({
                        sizes: toggleListValue<AbayaSize>(filters.sizes, size),
                      })
                    }
                    className={cn(
          ...
          <AccordionContent>
            <TooltipProvider delay={100}>
              <div className="flex flex-wrap gap-2.5 pt-1">
                {PRODUCT_SWATCH_IDS.map((swatch) => {
                  const selected = filters.colors.includes(swatch);
                  const colorName = tColors(swatch);
                  return (
                    <Tooltip key={swatch}>
                      <TooltipTrigger
                        type="button"
                        aria-label={t("panel.selectColor", { color: colorName })}
                        aria-pressed={selected}
                        onClick={() =>
                          update({
                            colors: toggleListValue<ProductSwatchId>(
                              filters.colors,
                              swatch,
                            ),
                          })
                        ...
                        update({
                          badges: isChecked
                            ...
                            : filters.badges.filter(
                                (item) => item !== (badge as ProductBadge),
                              ),
                        });
                      }}
                    />
      ...
      <div className="space-y-4 border-t border-border pt-4">
        ...
        <div className="flex items-center justify-between gap-3">
          <Label
            htmlFor={`${idPrefix}-includes-sheila`}
            className="font-normal"
          >
            {t("panel.includesSheila")}
          </Label>
          <Switch
            id={`${idPrefix}-includes-sheila`}
            checked={filters.includesSheila}
            onCheckedChange={(checked) => update({ includesSheila: checked })}
          />
        </div>
      </div>
