...
import { ProductPrice, SAMPLE_PRODUCTS } from "@/features/product";
import type { OrderItemSummary, OrderSummary } from "../types";
...
function getProductHref(item: OrderItemSummary): string | null {
  ...
  const match = SAMPLE_PRODUCTS.find(
    (p) =>
      p.slug === item.slug ||
      p.nameKey.toLowerCase() === item.name.toLowerCase().replace(/\s+/g, "") ||
      item.name.toLowerCase().includes(p.slug.split("-")[0] ?? "")
  );
...
function formatDate(locale: string, dateStr: string, style: "short" | "long" = "short") {
  try {
...
export async function OrderDetailsView({ order }: OrderDetailsViewProps) {
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            >
              {order.items.map((item, idx) => {
                ...
                return (
                  <StaggerItem key={`${order.id}-${item.name}-${idx}`}>
                            ...
                            >
                              {item.image ? (
                                ...
                                <div className="flex size-full items-center justify-center text-muted-foreground">
                                  <Package className="size-5 sm:size-7" aria-hidden />
                                </div>
                              )}
                            </Link>
                          ) : (
          ...
          <div className="grid gap-3 sm:grid-cols-2 sm:gap-4">
            {order.shippingAddress && (
              <div className="rounded-xl border border-border bg-card p-3 sm:rounded-2xl sm:p-5">
                <div className="mb-2 flex items-center gap-2 text-foreground sm:mb-3">
                  <MapPin className="size-3.5 text-muted-foreground sm:size-4" aria-hidden />
            ...
            <div className="rounded-xl border border-border bg-card p-3 sm:rounded-2xl sm:p-5">
              ...
              <dl className="space-y-1.5 text-xs sm:space-y-2 sm:text-sm">
                ...
                <div className="flex items-center justify-between gap-3">
                  <dt className="text-muted-foreground">{t("details.shipping")}</dt>
                  <dd className="text-foreground">
                    {shippingTRY === 0 ? (
                      <span className="text-success">{t("details.shippingFree")}</span>
                    ) : (
                      <ProductPrice
